...
export async function getLandingPage(preview) {
  const entries = await fetchGraphQL(
    `query {
        landingPageCollection(limit: 50, preview: ${preview ? "true" : "false"
    }) {
          items {
            ${LANDING_PAGE_FIELDS}
          }
...
export async function getSubpage(preview) {
  const entries = await fetchGraphQL(
    `query {
      subpageCollection(limit: 20, preview: ${preview ? "true" : "false"
    }) {
          items {
            ${SUBPAGE_FIELDS}
          }
...
export async function getContactPage(preview) {
  ...
  return entries?.data?.contactPageCollection?.items || [];
}